...
export default function BlogAdmin() {
  ...
  useEffect(() => {
    axios
      .get(`https://api.localfarmnepal.com/api/blogs`)
      .then((res) => {
        setBlogs(res.data);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(
        `https://api.localfarmnepal.com/api/blogs/${selected.id}`,
        formData,
        {
          headers: {
  ...
  const handleAddSubmit = async (e) => {
    ...
    try {
      await axios.post(`https://api.localfarmnepal.com/api/blogs`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      <table className="border-1 border-gray-300 table-fixed">
        <thead>
          <tr className="border-1 border-gray-300">
            ...
            <th className=" border-1 p-1 text-center break-words max-w-[200px]">
              Slug
            </th>
            <th className="border-1 p-1 text-center break-words max-w-[200px]">
              Introduction
        ...
        <tbody>
          {currentBlogs.map((blog, index) => (
            <tr key={index} className="border-1 border-gray-300">
              ...
              <td className="break-words max-w-[200px] border-1 p-1 text-center">
                <div
                  className="line-clamp-3 overflow-hidden text-left"
